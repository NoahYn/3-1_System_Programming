///////////////////////////////////////////////////////////////////////
// File Name : 2021202033_preforked_server.c							 	 //
// Date : 2023/05/16	 											 //
...
// Description : This file is source code for Assignment #3-1		 //
///////////////////////////////////////////////////////////////////////
...
struct s_history cli_history[10]; // recent client connection history
...
void sigHandler(int sig) {
	if (sig == SIGALRM) { // timer is over!
		...
		if (WIFEXITED(stat)) { // child is terminated normally
			...
			printf("[%.24s]\nIP : %s\nPort : %d\n", ctime(&t), cli_history[stat%10].IP, cli_history[stat%10].port);
			puts("=================================\n");	
		}
	}
...
int main() {
	struct sockaddr_in srv_addr, cli_addr; // address of server
	int sd; // socket descripter
	struct s_chld *chld_curr;
